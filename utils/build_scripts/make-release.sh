...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/furyproject/fury/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin fury-$build-x64-$version
    zip -r fury-$build-x64-$version.zip fury-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum fury-*-x64-$version.zip
