...
*Setup instructions for a Gitian build of Fury using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Fury CLI
...
```bash
cp fury/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```
./gitian-build.py --setup fury-user x.x.x
```

Where `fury-user` is your Github name and `x.x.x` is the version tag you want to build.
...
```
git clone git@github.com:fury-project/gitian.sigs.git
git remote add fury-user git@github.com:fury-user/gitian.sigs.git
```
...
To build the most recent tag:
 ...
 `./gitian-build.py --detach-sign --no-commit -b fury-user 2.0.0`
...
```
export NAME=fury-user
export VERSION=2.0.0
gpg --output $VERSION-linux/$NAME/fury-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/fury-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/fury-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/fury-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/fury-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/fury-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[fury-project/gitian.sigs](https://github.com/fury-project/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fury-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/fury-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fury-osx-*-build.assert
```
